...
const add_to_scraped = async (url, title, content,  brand_id, stock) => {
  try {
    ...
    const deletionResult = await delete_scraped_fromDB(brand_id);
...
const delete_scraped_fromDB = async (brand_id, stockName) => {
  try {
    const query = {
      brand_id: brand_id,
      content: { $in: ["", "Error fetching content"] },
    };
    if (stockName) {
    ...
    return `Deleted ${results.deletedCount} documents with brand`;
  } catch (error) {
